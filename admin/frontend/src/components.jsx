...
export function WindowControls() {
  ...
  return <div className="window-controls"><button data-tooltip="最小化" onClick={() => window.windowControls?.minimize()} aria-label="最小化"><Minimize2 size={14} /></button><button data-tooltip="最大化" onClick={() => window.windowControls?.toggleMaximize()} aria-label="最大化"><Maximize2 size={14} /></button><button className="window-close" data-tooltip="关闭" onClick={() => window.windowControls?.close()} aria-label="关闭"><X size={15} /></button></div>
}
...
export function BotAvatar({ bot, className = '' }) {
  ...
  return <div className={`bot-avatar ${className}`} data-tooltip={`${bot?.name || 'Bot'} 头像`} aria-label={`${bot?.name || 'Bot'} 头像`}>
...
export function RestartableStatus({ label, state, title, disabled, onRestart }) {
  return <button type="button" className={`status-pill restartable-status ${state}`} onClick={onRestart} disabled={disabled} aria-label={title} data-tooltip={title}><i /><span className="status-current">{label}</span><span className="restart-label">重启</span></button>
}
export function renderLogText(message, prefix = '') {
  ...
  return parts.map((part, index) => /^https?:\/\//i.test(part)
    ? <a key={`${prefix}-${part}-${index}`} className="log-link" href={part} onClick={(event) => { event.preventDefault(); openExternal(part) }} data-tooltip="打开链接">{part}</a>
...
export function ImageMessage({ image }) {
  ...
  return <span className="log-image-message"><button type="button" className="image-message-button" onClick={() => setOpen(value => !value)} aria-expanded={open}><ImageIcon size={14} /><span>{image.summary}</span><small>{open ? '收起' : '查看图片'}</small></button>{open && <span className="image-message-panel"><span className="image-message-preview">{failed || cache.error ? <span className="image-message-error">{image.truncated ? '日志里的图片链接已被 NoneBot 截断，且本地没有可用缓存' : '图片加载失败，请打开原链接查看'}</span> : image.truncated && !cache.url ? <span className="image-message-error">正在加载图片…</span> : <img src={image.truncated ? cache.url : image.url} alt={image.summary} loading="lazy" referrerPolicy="no-referrer" onError={() => setFailed(true)} />}</span><span className="image-message-meta">{image.file}{image.size ? ` · ${image.size} bytes` : ''}</span><span className="image-message-url"><a href={image.url} onClick={(event) => { event.preventDefault(); openExternal(image.url) }} data-tooltip="打开原链接">{image.url}</a></span><span className="image-message-actions"><a className="image-message-action" href={image.url} onClick={(event) => { event.preventDefault(); openExternal(image.url) }}><ExternalLink size={13} />原链接</a><a className="image-message-action" href={image.url} onClick={saveImage}><Download size={13} />保存图片</a></span></span>}</span>
}
